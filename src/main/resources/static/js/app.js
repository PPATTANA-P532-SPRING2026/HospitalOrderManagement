        ...
        .then(data => {
            ...
            data.forEach(order => {
                ...
                row.innerHTML = `
                    ...
                    <td class="status-${order.status.toLowerCase().replace('_', '')}">
...
function buildActions(order) {
    if (order.status === 'PENDING') {
        return `
            ...
            <button class="btn-cancel" onclick="cancelOrder('${order.id}', '${order.clinician}')">
                Cancel
            </button>
        `;
    }
    ...
    if (order.status === 'IN_PROGRESS') {
        return `
            <button class="btn-complete" onclick="completeOrder('${order.id}', '${order.claimedBy || ''}')">
                Complete
            </button>
        `;
    }
    ...
    if (order.status === 'COMPLETED') {
        return `<span style="color:#1E7145;">Done</span>`;
    }
...
function submitOrder() {
    ...
    if (!clinician) {
        showMessage('submitMessage', 'Please select a clinician', 'error');
        return;
    }
    ...
    if (!patientName) {
        showMessage('submitMessage', 'Please enter patient name', 'error');
        return;
    }

    if (!description) {
        showMessage('submitMessage', 'Please enter description', 'error');
        return;
    }
    ...
    fetch(`${API_BASE}/api/orders`, {
        ...
        .then(res => {
            if (res.ok) {
                showMessage('submitMessage', 'Order submitted successfully', 'success');
                clearForm();
                refreshAll();
            } else {
                res.text().then(err => showMessage('submitMessage', err, 'error'));
            }
        })
...
function claimOrder(orderId) {
    ...
    if (strategy !== 'loadBalancing') {
        const claimedBy = document.getElementById('staffName').value;
        if (!claimedBy) {
            showMessage('claimMessage', 'Please select a staff member', 'error');
            return;
        }
        body.claimedBy = claimedBy;
    }
    ...
    fetch(`${API_BASE}/api/orders/${orderId}/claim`, {
        ...
        .then(res => {
            if (res.ok) {
                showMessage('claimMessage', 'Order claimed successfully', 'success');
                refreshAll();
            } else {
                res.text().then(err => showMessage('claimMessage', err, 'error'));
            }
        })
...
function completeOrder(orderId, claimedBy) {
    fetch(`${API_BASE}/api/orders/${orderId}/complete`, {
        ...
        .then(res => {
            if (res.ok) {
                showMessage('claimMessage', 'Order completed by ' + claimedBy, 'success');
                refreshAll();
            } else {
                res.text().then(err => showMessage('claimMessage', err, 'error'));
            }
...
function cancelOrder(orderId, clinician) {
    if (!clinician) {
        showMessage('claimMessage', 'No clinician found for this order', 'error');
        return;
    }
    ...
    fetch(`${API_BASE}/api/orders/${orderId}`, {
        ...
        .then(res => {
            if (res.ok) {
                showMessage('claimMessage', 'Order cancelled successfully', 'success');
                refreshAll();
            } else {
                res.text().then(err => showMessage('claimMessage', err, 'error'));
            }
...
function fetchAuditLog() {
    fetch(`${API_BASE}/api/audit`)
        ...
        .then(data => {
            ...
            if (data.length === 0) {
                tbody.innerHTML = '<tr><td colspan="5">No actions yet</td></tr>';
                return;
            }
            ...
            [...data].reverse().forEach(entry => {
                ...
                row.innerHTML = `
                    ...
                    <td>${entry.actor}</td>
                    <td>
                        <button class="btn-claim" onclick="replayCommand('${entry.id}')">
                            Replay
                        </button>
                    </td>
                `;
...
function undoLast() {
    fetch(`${API_BASE}/api/audit/undo`, { method: 'POST' })
        .then(res => {
            if (res.ok) {
                showMessage('undoMessage', 'Last command undone', 'success');
                refreshAll();
            } else {
                res.text().then(err => showMessage('undoMessage', err, 'error'));
            }
        })
...
function replayCommand(entryId) {
    fetch(`${API_BASE}/api/audit/replay/${entryId}`, { method: 'POST' })
        .then(res => {
            if (res.ok) {
                showMessage('undoMessage', 'Command replayed successfully', 'success');
                refreshAll();
            } else {
                res.text().then(err => showMessage('undoMessage', err, 'error'));
            }
        })
        .catch(err => console.error('Error replaying:', err));
}
...
function changeStrategy() {
    ...
    fetch(`${API_BASE}/api/strategy`, {
        ...
        .then(res => res.json())
        .then(() => {
            showMessage('strategyMessage', 'Strategy changed to ' + strategy, 'success');
...
function updateChannels() {
    ...
    fetch(`${API_BASE}/api/notifications/channels`, {
        ...
        .then(res => res.json())
        .then(() => {
            showMessage('notificationMessage', 'Channels updated', 'success');
        })
...
// ── reset badge ───────────────────────────────────────────────────────
function resetBadge() {
    fetch(`${API_BASE}/api/notifications/badge/reset`, { method: 'POST' })
        .then(res => res.json())
        .then(() => {
...
function registerClinician() {
    const name = document.getElementById('clinicianNameReg').value;
    const department = document.getElementById('clinicianDept').value;

    if (!name) {
        showMessage('clinicianMessage', 'Please enter clinician name', 'error');
        return;
    }
    ...
    fetch(`${API_BASE}/api/clinicians`, {
        ...
        .then(res => {
            if (res.ok) {
                showMessage('clinicianMessage', name + ' registered successfully', 'success');
                document.getElementById('clinicianNameReg').value = '';
                fetchClinicians();
            } else {
                res.text().then(err => showMessage('clinicianMessage', err, 'error'));
            }
        })
        .catch(err => console.error('Error registering clinician:', err));
}
...
function fetchClinicians() {
    fetch(`${API_BASE}/api/clinicians`)
        .then(res => res.json())
        .then(data => {
            const tbody = document.getElementById('clinicianTable');
            tbody.innerHTML = '';

            if (data.length === 0) {
                tbody.innerHTML = '<tr><td colspan="2">No clinicians registered</td></tr>';
            } else {
                data.forEach(c => {
                    const row = document.createElement('tr');
                    row.innerHTML = `
            ...
            if (currentVal) {
                clinicianSelect.value = currentVal;
            }
        })
...
function registerStaff() {
    const name = document.getElementById('staffNameReg').value;
    const role = document.getElementById('staffRole').value;

    if (!name) {
        showMessage('staffMessage', 'Please enter staff name', 'error');
        return;
    }
    ...
    fetch(`${API_BASE}/api/staff`, {
        ...
        .then(res => {
            if (res.ok) {
                showMessage('staffMessage', name + ' registered successfully', 'success');
                document.getElementById('staffNameReg').value = '';
                fetchStaff();
            } else {
                res.text().then(err => showMessage('staffMessage', err, 'error'));
            }
        })
        .catch(err => console.error('Error registering staff:', err));
}
...
function fetchStaff() {
    fetch(`${API_BASE}/api/staff`)
        ...
        .then(data => {
            ...
            if (data.length === 0) {
                tbody.innerHTML = '<tr><td colspan="2">No staff registered</td></tr>';
            } else {
                data.forEach(s => {
                    const row = document.createElement('tr');
                    row.innerHTML = `
